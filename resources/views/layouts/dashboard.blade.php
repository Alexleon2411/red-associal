    ...
    <div class="flex justify-center">
        <div class="w-full md:w-8/12 lg:w-6/12 flex flex-col items-center md:flex-row">
            ...
            <div class="md:w-8/12 lg:w-6/12 px-5 flex flex-col items-center md:items-start md:justify-center ml-5">
                ...
                <p class="text-gray-800 text-2xl font-bold mt-5">{{$user->followers()->count()}}  <span class="font-normal">@choice('Seguidor|Seguidores', $user->followers()->count())</span></p>
                <p class="text-gray-800 text-2xl font-bold">{{$user->followings()->count()}}  <span class="font-normal">Siguiendo</span></p>
                <p class="text-gray-800 text-2xl font-bold">{{$posts->count()}} <span class="font-normal">Post</span></p>
                @auth
                    @if($user->id !== auth()->user()->id)
                        ...
                        <form  method="POST" action="{{route('users.unfollow', $user)}}">
                            @method('DELETE')
                            @csrf
                            <input type="submit" value="Dejar de Seguir" class="bg-indigo-500 hover:bg-indigo-700 transition-colors cursor-pointer uppercase font-bold w-full px-2 text-white rounded-lg">
                        </form>
                        @else
                        <form  method="POST" action="{{route('users.follow', $user)}}">
                            @csrf
                            <input type="submit" value="Seguir" class=" bg-rose-500 hover:bg-pink-800  transition-colors cursor-pointer uppercase font-bold w-full px-2 text-white rounded-lg my-2">
                        </form>
                    ...
                    <a
                      ...
                      class="bg-gray-600 text-white uppercase rounded-lg px-3 py-1 text-xs font-bold cursor-pointer inline-block">
                        Enviar Mensaje
                    </a>
                @endauth
            </div>
